...
const SeamlessTerrainRenderer = ({ 
  ...
  useEffect(() => {
    ...
    if (targetPosition) {
      renderTarget(ctx, targetPosition, cellSize);
    }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      if (tile.baseTexture && loadedImages.has(tile.baseTexture)) {
        ...
        // Render with seamless borders (no gaps)
        ctx.drawImage(
          img,
          pixelX,
          pixelY,
...
const renderTarget = (ctx, targetPosition, cellSize) => {
  if (!targetPosition) return;
  
  const pixelX = targetPosition.x * cellSize + cellSize / 2;
  const pixelY = targetPosition.y * cellSize + cellSize / 2;
